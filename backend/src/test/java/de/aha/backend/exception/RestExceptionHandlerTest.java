...
class RestExceptionHandlerTest {
    ...
    void buildResponse_ShouldCreateCorrectResponseEntity() {
        ...
        assertTrue(response.getBody().getMessage().contains("Test error message"));
    ...
    void handleHttpClientErrorException_ShouldReturnUnauthorized() {
        ...
        assertTrue(response.getBody().getMessage().contains("Unauthorized")); // Проверяем contains
        assertTrue(response.getBody().getMessage().contains("access")); // Проверяем contains
    ...
    void handleBadCredentialsException_ShouldReturnUnauthorized() {
        ...
        assertNotNull(response.getBody());
        assertEquals(ErrorType.AUTHORIZATION_ERROR.name(), response.getBody().getCode());
        assertTrue(response.getBody().getMessage().contains("Bad credentials")); // Проверяем contains
        assertEquals(401, response.getBody().getStatus());
    }
